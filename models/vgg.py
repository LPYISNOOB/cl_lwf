...
class MultiHeadVGGSmall(MultiTaskModule):
    ...
    def forward(self, x, task_labels):
        ...
        # x = torch.flatten(x, 1)
        #
        # # 确定张量的形状，并在 torch.zeros 中使用常量
        # batch_size = x.shape[0]
        # output_features = self.classifier.starting_out_features  # 假设这是 MultiHeadClassifier 中定义的属性
        # zeros_shape = (batch_size, output_features)
        #
        # # 使用确定的形状创建全零张量
        # out = torch.zeros(zeros_shape, device=x.device)

        # 调用分类器，传入特征和任务标签
        # return self.classifier(x, task_labels, out)
